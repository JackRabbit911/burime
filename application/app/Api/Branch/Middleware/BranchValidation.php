...
class BranchValidation extends ApiContractValidation
{
    protected function setRules($request)
    {
        $this->validation
            ...
            ->rule('branch.cover.bg_color', 'hex_color')
            ->rule('branch.cover.text_color', 'hex_color')
            ->rule('branch.cover.text_size', 'required|integer|inRange(5, 50)')
            ->rule('branch.cover.bg_img', 'text_utf8')
            ->rule('branch.cover.cover', 'text_utf8')
